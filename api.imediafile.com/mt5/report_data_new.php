<?php
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://client.3anglefx.com/api.adjustment-excluded");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$r = curl_exec($ch);
if (curl_getinfo($ch, CURLINFO_HTTP_CODE) != 200) die('CURL_ERROR');
$adjToExc = json_decode($r, true);

if (isset($_REQUEST['view']) && $_REQUEST['view'] == 1) {
    header('Content-Type: text/html; charset=utf-8');
    if (empty($_SERVER['PHP_AUTH_USER']) ||
        $_SERVER['PHP_AUTH_USER'] != "joe_test" ||
        $_SERVER['PHP_AUTH_PW'] != "RNDD8eUdHdfPZqTX") {
    ...
    if (!$isTrusted) die(json_encode(["code" => 200, "status" => "ERROR", "message" => "WHITELIST_ERROR", "detail" => null]));
}
/* -------------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------- */
// CACHE
$sql = "SELECT `data` FROM `rates`.`report_history` WHERE `link` = '{$_REQUEST['from']}|{$_REQUEST['to']}|{$_REQUEST['brand']}'";
$result = $mysqli2->query($sql);
if ($result->num_rows !== 0) {
    $row = mysqli_fetch_assoc($result);
    die(str_replace('n/a', 'n\/a', str_replace("\\", "\\\\", $row['data'])));
}
/* -------------------------------------------------------------------------- */
//DEVEXPERTS
if ($_REQUEST['brand'] === 'coinfy.io' || $_REQUEST['brand'] === 'bidaskbit.com' || $_REQUEST['brand'] === 'oipax.com' || $_REQUEST['brand'] === 'spottop.co.uk') {
    if ($_REQUEST['from'] < '2023-02-22') {
        die(json_encode(["code" => 200, "status" => "ERROR", "message" => "MIN_DATE_ERROR", "detail" => null]));
    }
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "https://payment.imediafile.com/report/dx/?token=AQxbzuVw7E7anpFj&brand={$_REQUEST['brand']}&from={$_REQUEST['from']}&to={$_REQUEST['to']}");
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    $r = curl_exec($ch);
    die($r);
}
...
while ($rowLogin = mysqli_fetch_assoc($resultLogin)) {
    $resultArray[$rowLogin['TPA']] = array(
        ...
        'usd_nb' => 0, 'origin_nb' => 0, 'origin_pnl_credit_start' => 0, 'origin_pnl_credit_end' => 0, 'usd_pnl_credit_start' => 0, 'usd_pnl_credit_end' => 0, 'currency_precision' => '2',
    );
}
/* -------------------------------------------------------------------------- */
// REALIZED + SWAP
...
//WHERE mt5_deals.Symbol != '' AND mt5_users.`Group` $str AND  mt5_deals.Time > '" . $nativeFrom . " 00:00:00' AND mt5_deals.Time < '" . $nativeTo . " 23:59:59';";

$sql = "SELECT 
mt5_deals.Login AS 'TPA', mt5_deals.Time AS 'DT', 
mt5_deals.Profit AS 'PROFIT', mt5_deals.Storage AS 'SWAP', 
mt5_deals.Commission AS 'COMMISSION', 
((mt5_deals.MarketAsk - mt5_deals.MarketBid) / 2) * mt5_deals.ContractSize * (mt5_deals.Volume / 10000) AS 'SPREAD',
mt5_symbols.CurrencyProfit AS 'SPREAD_CURRENCY'
FROM mt5_deals JOIN mt5_symbols ON (mt5_deals.Symbol = mt5_symbols.Symbol) JOIN mt5_users ON (mt5_deals.Login = mt5_users.Login)
WHERE mt5_deals.Symbol != '' AND mt5_users.`Group` $str AND  mt5_deals.Time > '" . $nativeFrom . " 00:00:00' AND mt5_deals.Time < '" . $nativeTo . " 23:59:59';";
/* -------------------------------------------------------------------------- */
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    if (str_contains($row['COMMENT'], 'ad_') && !in_array($row['COMMENT'], $adjToExc)) {
...
/* -------------------------------------------------------------------------- */
// START LOSS
$sql = "SELECT IF(mt5_daily.Credit - mt5_daily.ProfitEquity < 0, 0, mt5_daily.Credit - mt5_daily.ProfitEquity) AS 'FINALPNL_CREDIT_VALUE', FROM_UNIXTIME(mt5_daily.Datetime) AS DT, mt5_daily.Profit + mt5_daily.ProfitStorage AS 'START_PROFIT_LOSS', mt5_daily.Credit AS 'CREDIT', mt5_daily.Login AS 'TPA' 
...
//$ecbRateDate = explode('-', explode(' ', $row['DT'])[0]);
    ...
    $resultArray[$row['TPA']]['usd_credit_start'] = ($row['FINALPNL_CREDIT_VALUE'] / $ecbRate);
    $resultArray[$row['TPA']]['origin_credit_start'] = $row['FINALPNL_CREDIT_VALUE'];
}
/* -------------------------------------------------------------------------- */
/* -------------------------------------------------------------------------- */
/* -------------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------- */
// END LOSS
$sql = "SELECT IF(mt5_daily.Credit - mt5_daily.ProfitEquity < 0, 0, mt5_daily.Credit - mt5_daily.ProfitEquity) AS 'FINALPNL_CREDIT_VALUE', FROM_UNIXTIME(mt5_daily.Datetime) AS DT, mt5_daily.Balance AS 'BALANCE', mt5_daily.EquityPrevDay AS 'EQUITY', mt5_daily.Profit + mt5_daily.ProfitStorage AS 'END_PROFIT_LOSS', mt5_daily.Credit AS 'CREDIT', mt5_daily.Login AS 'TPA' 
...
//$ecbRateDate = explode('-', explode(' ', $row['DT'])[0]);
    ...
    $resultArray[$row['TPA']]['usd_credit_end'] = ($row['FINALPNL_CREDIT_VALUE'] / $ecbRate);
    $resultArray[$row['TPA']]['origin_credit_end'] = $row['FINALPNL_CREDIT_VALUE'];
}
/* -------------------------------------------------------------------------- */
/* -------------------------------------------------------------------------- */
/* -------------------------------------------------------------------------- */
...
foreach ($resultArray as $key => $val) {
    $resultArray[$key]['usd_final_pnl'] = (($val['usd_end'] - $val['usd_start']) + $val['usd_realized_pnl_plus_swap'] + $val['usd_dividend'] + $val['usd_commission'] + $val['usd_adjustment'] + $val['usd_nb'] + ($val['usd_credit_end'] - $val['usd_credit_start']));
    $resultArray[$key]['origin_final_pnl'] = (($val['origin_end'] - $val['origin_start']) + $val['origin_realized_pnl_plus_swap'] + $val['origin_dividend'] + $val['origin_commission'] + $val['origin_adjustment'] + $val['origin_nb'] + ($val['origin_credit_end'] - $val['origin_credit_start']));
}
...
mt5_deals.`COMMENT` NOT LIKE 'it_%' AND
mt5_deals.`COMMENT` NOT LIKE '%Test%'
) AND mt5_deals.Profit > 0 AND mt5_deals.Time > '$nativeFrom 00:00:00' AND mt5_deals.Time < '$nativeTo 23:59:59' 
...
mt5_deals.`COMMENT` NOT LIKE 'it_%' AND
mt5_deals.`COMMENT` NOT LIKE '%Test%'
) AND mt5_deals.Profit > 0 AND mt5_deals.Time > '2020-09-01 00:00:00' AND mt5_deals.Time < '$nativeTo 23:59:59' 
GROUP BY mt5_deals.Login;";
...
<td>FD(usd/tac)</td><td>FD time</td><td>withdraw(usd/tac)</td><td>negative balance(usd/tac)</td><td>FINAL_PNL_SWAP(usd/tac)</td><td>FORMULA</td></tr>';
    foreach ($resultArray as $key => $val) {

        if (isset($_REQUEST['account'])) {
            if ($key != $_REQUEST['account']) {
                continue;
            }
        }
...
//}
        ...
        echo '<td>( END: ' . $val['origin_end'] . ' - START: ' . $val['origin_start'] . ') + REALIZED_SWAP: ' . $val['origin_realized_pnl_plus_swap'] . ' + DIVIDEND: ' . $val['origin_dividend'] . ' + COMMISSION: ' . $val['origin_commission'] . ' + ADJUSTMENT: ' . $val['origin_adjustment'] . ' + NEGATIVE_BALANCE: ' . $val['origin_nb'] . ' + ( CREDIT_END: ' . $val['origin_credit_end'] . ' - CREDIT_START: ' . $val['origin_credit_start'] . ')</td>';
        echo '</tr>';
    }
    echo '</table>';
    die();
...
/* -------------------------------------------------------------------------- */
// CACHE
$result = $mysqli2->query("INSERT INTO `report_history`(`link`, `data`) VALUES ('{$_REQUEST['from']}|{$_REQUEST['to']}|{$_REQUEST['brand']}','" . json_encode($resultArray) . "');");
/* -------------------------------------------------------------------------- */
